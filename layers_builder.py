...
class Lambda(layers.Layer):
    ...
    def build(self, input_shape):
        super(Lambda, self).build(input_shape)
    ...
    def get_config(self):
        config = super(Lambda, self).get_config()
...
def interp_block(prev_layer, level, feature_map_shape, input_shape):
    ...
    prev_layer = Lambda(feature_map_shape)(prev_layer)
    return prev_layer
...
def build_pspnet(nb_classes, resnet_layers, input_shape, activation='softmax'):
    ...
    x = Lambda([input_shape[0], input_shape[1]])(x)
